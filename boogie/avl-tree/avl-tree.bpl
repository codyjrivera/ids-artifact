...
) returns (bool)
{
    (x != null ==> (
        ...
        && (l[x] == null && r[x] == null ==>
                height[x] == 1)
        && (l[x] != null && r[x] == null ==>
                height[x] == height[l[x]] + 1
                && height[l[x]] == 1)
        && (l[x] == null && r[x] != null ==>
                height[x] == height[r[x]] + 1
                && height[r[x]] == 1)
        && (l[x] != null && r[x] != null ==>
                (height[l[x]] > height[r[x]] ==>
                    height[x] == height[l[x]] + 1)
                && (height[l[x]] <= height[r[x]] ==>
                        height[x] == height[r[x]] + 1)
                && -1 <= height[r[x]] - height[l[x]]
                && height[r[x]] - height[l[x]] <= 1)
    ))
}

function {:inline} LC_Trans_Unbalanced(
    k: [Ref]int, 
    l: [Ref]Ref,
    r: [Ref]Ref,
    p: [Ref]Ref,
...
) returns (bool)
{
    (x != null ==> (
        ...
        && (l[x] != null && r[x] != null ==>
                ...
                && (height[l[x]] <= height[r[x]] ==>
                        height[x] == height[r[x]] + 1)
                && -2 <= height[r[x]] - height[l[x]]
                && height[r[x]] - height[l[x]] <= 2)
    ))
}
...
procedure Get_l(x: Ref) returns (l: Ref);
    requires x != null;
    ensures l == C.l[x];
    ensures InAlloc(C.k, C.l, C.r, C.p, C.height, 
...
procedure Get_height(x: Ref) returns (height: int);
    requires x != null;
    ensures height == C.height[x];
...
procedure Set_height(x: Ref, height: int);
    requires x != null;
    modifies Br, C.height;
    ensures C.height == old(C.height)[x := height];
    ensures C.p[x] != null ==> Br == (old(Br)[x := true])[C.p[x] := true];
    ensures C.p[x] == null ==> Br == old(Br)[x := true];
...
procedure IfNotBr_ThenLC(x: Ref);
    ensures x != null && !Br[x] ==> LC(C.k, C.l, C.r, C.p, C.height, 
                                        C.min, C.max, C.keys, C.repr,
                                        x);

procedure AssertLCAndRemove(x: Ref);
    modifies Br;
    ensures (x != null && LC(C.k, C.l, C.r, C.p, C.height, 
                            C.min, C.max, C.keys, C.repr,
                            x)) 
                ==> Br == old(Br)[x := false];
    ensures (x == null || !LC(C.k, C.l, C.r, C.p, C.height, 
...
function {:inline} Frame_height(mod_set: RefSet, old_height: [Ref]int, height: [Ref]int) returns ([Ref]int)
{
    MapIte_Ref_int(mod_set, height, old_height)
}
...
) returns (bool) 
{
    ...
    && height == Frame_height(RefSetUnionF(mod_set, RefSetComF(old_alloc)), oldheight, height)
...
procedure InAllocParam(x: Ref);
    ensures x != null ==> InAlloc(C.k, C.l, C.r, C.p, C.height,
                                C.min, C.max, C.keys, C.repr,
                                x, alloc);
